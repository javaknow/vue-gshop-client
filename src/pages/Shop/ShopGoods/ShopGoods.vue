  ...
  methods:{
    _initScroll () {
      ...
      this.rightScroll.on('scroll', ({x, y}) => {
        console.log('rightScroll.scroll', x, y)
        this.scrollY = Math.abs(y)
      })

      // 监视右侧列表滑动结束
      this.rightScroll.on('scrollEnd', ({x, y}) => {
        console.log('rightScroll.scrollEnd', x, y)
        this.scrollY = Math.abs(y)
      })
    },
    _initTops () {
      ...
      let top = 0
      tops.push(top)
      const lis = this.$refs.rightUl.children; //document.getElementsByClassName('food-list-hook')
